...
export const ShowcaseGrid = ({
  ...
  return (
    <ul aria-label={t('commerce:grid.label')} className="commerce-product-grid">
      {products.filter(isPurchasable).map((product) => {
        ...
        const reason = ownedUnique
          ? t('commerce:grid.reasonOwned')
          : otherCurrency
            ? t('commerce:grid.reasonCurrency', { currency: cartCurrency })
            : suspended
              ? t('commerce:grid.reasonSuspended')
              : soldOut
                ? t('commerce:grid.reasonSoldOut')
                : undefined
        return (
          <li key={product.productId} className="min-h-0 min-w-0">
            ...
            >
              <button
                type="button"
                aria-label={t('commerce:grid.openDetail', { name: product.name })}
                onClick={() => {
              ...
              {onToggleWish !== undefined && (
                <button
                  ...
                  aria-label={
                    wished
                      ? t('commerce:grid.wishRemove', { name: product.name })
